...
function Reviews(props) {
  ...
  return (
    <div className="container-fluid p-0">
      <div className="row m-0" style={{ minHeight: "500px" }}>

        {/* Left: Reviews */}
        <div
          className="col-12 col-lg-6 p-0 d-flex flex-column justify-content-center bg-dark"
        >
          <h1 className="text-secondary text-center mt-5 fw-bold">
            Hear Our Clients Thoughts
          </h1>
          <div className="my-4 py-3 d-flex justify-content-center">
...
function Item(props) {
  return (
    <Paper elevation={0}>
      <div className="text-center text-white bg-dark px-3">
        <p>{props.item.review}</p>
        <h2 className="fw-bold">{props.item.name}</h2>
        <p className="text-secondary">{props.item.company}</p>
        <br />
      </div>
    </Paper>
  );
